...
public class Hoe {
	public static void main(String[] args) throws IOException {
		BufferedImage bi = ImageIO.read(Hoe.class.getResourceAsStream("/net/lesmi/iron_hoe.png"));
		int w = bi.getWidth();
		int h = bi.getHeight();
	...
	public static void processForColor(int w, int h, int[] basePixels, int mask, File file) {
		if (file == null)
			file = new File("hoe_color_" + Integer.toHexString(mask).substring(1) + ".png");
		int[] px = Arrays.copyOf(basePixels, basePixels.length);
		Pickaxe.process2(px, 7, 1, w, mask);
		Pickaxe.process2(px, 8, 1, w, mask);
		Pickaxe.process2(px, 9, 1, w, mask);

		Pickaxe.process2(px, 6, 2, w, mask);
		Pickaxe.process2(px, 7, 2, w, mask);
